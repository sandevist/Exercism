        ...
        while i < len(tokens):
            ...
            if token == ':':
                ...
                while j < len(tokens) and tokens[j] != ';':
                    ...
                    # EXPAND DEFINITIONS IMMEDIATELY - KEY FIX
                    if subtoken in definitions:
                        # If the token is a defined word, expand it now
            ...
            if token in definitions:
                ...
                while k < len(def_tokens):
                    ...
                    if def_token in definitions:
                        ...
                        nested_tokens = definitions[def_token]
                        m = 0
                        while m < len(nested_tokens):
                            process_token(nested_tokens[m].lower(), stack, definitions)
                            m += 1
                    else:
...
def process_token(token, stack, definitions):
    ...
    if token == '+':
        ...
        # Forth uses integer division that truncates toward zero
        result = a // b
        # Adjust for negative division (truncate toward zero, not floor)
        if a * b < 0 and a % b != 0:
            result += 1
        stack.append(result)
